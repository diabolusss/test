...
public class WSocketServer {
    ...
    //is server connection secured
    public static Boolean tls_enabled_server    = false;
    
    //send response to clients in json format?
    public static Boolean json_response_enabled = false;
    
    //is server response to client encoded?
    public static Boolean response_encode_enabled = false;
    ...
    //after sending ping server must receive pong
    // if it doesn't receive x times -> remove user from active list
    static Timer pinger;
    private static long PING_INTERVAL = 15; //in sec
    
    
    //private static String dtFormat;
    /**
     * @param args server_settings
     */
    public static void main(String[] args){
        ...
        if(tls_enabled_server){
            ...
            try {
                ...
                tls_enabled_server=false;
            }
        }
        /** SECURE PART GOES THERE **/
        //notify db about server status
        DBMakeQuery.setWebsocketTLS(jdbc, null, (tls_enabled_server==true)?(1):(0), server_host);    
        //clear online user counter
        DBMakeQuery.setAllOffline(jdbc, null, 0);
        DBMakeQuery.setAllOffline(jdbc, jdbc_demo_name, 0);
        ...
        pinger = new Timer();
        pinger.scheduleAtFixedRate(new Pinger(), 0, TimeUnit.SECONDS.toMillis(PING_INTERVAL));
        
        Functions.printLog("SERVER[INIT]: INF - Some bash command["+BASH_CMD+"]");                        
        try {
            Runtime.getRuntime().exec(BASH_CMD);
            Functions.printLog("SERVER[INIT]: INF - SERVER Status manager started");           

        } catch (IOException ex) {
            Functions.printLog("SERVER[INIT]: ERR - Failed to start SERVER Status manager");           
        }
    }
    ...
    private static void initVariables(Properties properties){
        ...
        //this variable can be omitted, because every user specifies server response type
        //json_response_enabled = Boolean.parseBoolean(properties.getProperty("json_response"));
        
        PING_INTERVAL = Integer.parseInt(properties.getProperty("ping_interval"));
        ...
        if(tls_enabled_server && (STORETYPE == null || KEYSTORE == null)){
            Functions.printLog("FIXPROCESS:[initVariables] ERR - Not defined keystore or keystore type.");
            //halt program or continue
            //return;
        }
        ...
        BASH_CMD = properties.getProperty("run_bash");
        Functions.printLog("SERVER:[initVariables] INF - some bash command["+BASH_CMD+"]");
        
        Functions.printLog("SERVER:[initVariables] INF - Properties parsed successfully");   
    }
