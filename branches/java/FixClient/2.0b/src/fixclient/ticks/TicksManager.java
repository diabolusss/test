...
public class TicksManager extends Thread{
    ...
    static int TOSF = 0; // tick occurence status flag -> if 1 everything is good
    static int TOCNT = 0; // tick occurence counter -> counts up if tick wasnt obtained
    ...
    public TicksManager(Properties properties){
        ...
        Functions.printLog("TM[INIT]: INF - Some bash command["+FixClient.BASH_CMD+"]");                        
        try {
            Runtime.getRuntime().exec(FixClient.BASH_CMD + " restart");
            Functions.printLog("TM[INIT]: INF - FIX Status manager started");

        } catch (IOException ex) {
            Functions.printLog("TM[INIT]: ERR - Failed to start FIX Status manager");           
        }
    }
    ...
    public synchronized static void newTick(double new_p){ 
        ...
        Functions.printLog("TM[newTick]: INF - New price["+price+"]; TOCNT="+TOCNT);
        TOSF = 1;
        TOCNT = 0;
    ...
    public void run() {
        ...
        while (true){ 
           ...
           Functions.printLog("TM[Run]: INF - Getting new mix_price Beep! "+mix_on+","+TOCNT+","+TicksManager.mix_price+","+TicksManager.real_price);
           if ("mix".equals(TicksManager.mode) && mix_on && (TOCNT < 50)){ // && (TicksManager.serverStatus == 0)) {                 
                //init value
                if (mix_price < 0.1D) {
           ...
           ++TOCNT;
            ...
            if (
                ...
                */
                ...
                //check
                //TODO add cheats price offsets
                if(isBreakPoint(totalOrderTypeBreakPoints, TicksManager.price)){
                    //NB REVISIONED
                    DBMakeQuery.callAutoCloseOrdersByUserID(jdbc_connection, null, price);
                    DBMakeQuery.callAutoCloseOrdersByUserID(jdbc_connection, FixClient.jdbc_demo_name, price);
                }
                ...
                if(FixClient.dukasFix != null && FixClient.dukasFix.started){  
                    //send orders to fix if its started
                    //NB REVISIONED
                    ...
                    tResult = DBMakeQuery.callGetForMarketClose(jdbc_connection, null);
                    if(tResult != null){
                        int count = Integer.parseInt(tResult.split("#")[0]);
                        double amount = Double.parseDouble(tResult.split("#")[1]);
                        int clordid = Integer.parseInt(tResult.split("#")[2]);
                ...
                DBMakeQuery.battleEffectDeactivate(jdbc_connection, FixClient.jdbc_demo_name);
                //DBMakeQuery.battleEffectDeactivate(jdbc_connection, null);
                ...
                for (URI serv_uri:WSConnectionList.keySet()){
                    ...
                    try {
                        //conn.setSocket(FixClient.SecureSocketFactory.createSocket());                    
                        //conn.connect();
                        //conn.connectBlocking();
                        if(conn.isOpen()){
                            String msg = null;
                            /*"FIXPrice:"+TicksManager.price
                                    +":"+bar_id_demo
                            ...
                            msg = "{\"type\":\"rate\",\"msg\":{"
                                        + "\"src\":\""+ FixClient.rateSource+"\""
                                        + ", \"barid\":"+ bar_id_demo 
                                        + ", \"buysell\":\""+TicksManager.totalOrders+"\""
                                        + ", \"price\":"+TicksManager.price
                                        + "}"
                                    +"}";
                            Functions.printLog("TM:[run] INF - Sending msg["+msg+"] to ["+serv_uri.toString()+"]");
                            ...
                            Functions.printLog("TM:[run] WRN - Socket.getReadyState["+conn.getReadyState()+"] to ["+serv_uri.toString()+"]");
                            
                        }

                    } catch (Exception ex) {
                        Functions.printLog("TM:[run] WRN - Failed to send message. Exception: " + ex.getMessage());
                        continue;
                    }                    
    ...
    boolean isBreakPoint(ConcurrentHashMap<String, ArrayList<ArrayList<Double>>> totalOrderTypeBreakPoints, double price){
        ...
        for(String type:totalOrderTypeBreakPoints.keySet()){
            ...
            double 
                    ...
                    cheat_price = 0;
            while(iter.hasNext()){   
                loss = price_set.get(i).get(0);
                profit = price_set.get(i).get(1);
                cheat_price = price_set.get(i).get(2);
                
                Functions.printLog("TM:[isBreakPoint] INF - "+type+" - [loss:"+loss+", profit:"+profit+", cheat_price:"+cheat_price+"] currPrice: "+price);
                                
                //if buy then if price >= profit or price <= loss then return true                
                if(type.equalsIgnoreCase("buy") ){
                    if (cheat_price > 0){
                        if(loss >= cheat_price || cheat_price >= profit)
                            return true;
                        
                    } else if(loss >= price || price >= profit) 
                        return true;
                    
                //if sell then if price <= profit or price >= loss then return true
                } else if(type.equalsIgnoreCase("sell")) {
                    if (cheat_price > 0){
                        if(profit >= cheat_price || cheat_price >= loss)
                            return true;
                        
                    } else if(profit >= price || price >= loss) 
                        return true;
    ...
    class SocketChecker extends TimerTask{
        ...
        public void run() {   
            for (URI serv_uri:WSConnectionList.keySet()){
                ...
                if(!conn.isOpen()){                    
                    Functions.printLog("DELAYER:[run] INF - Restoring connection to "+serv_uri.toString()+"; "+ serv_uri.getScheme());
                    SimpleClient client = new SimpleClient(serv_uri);
                    try {
                        if(
                                //FixClient.tls_enabled_server
                                serv_uri.getScheme().equalsIgnoreCase("wss")
                                ) client.setSocket(FixClient.SecureSocketFactory.createSocket());                    
    ...
    class Scheduler extends TimerTask{
        ...
        public void run() {
            ...
            }else if(FixClient.autoFix != null && FixClient.autoFix.running == 1){
                return;
            }
            ...
            if((TicksManager.serverStatus == 0) && server_stop.isBefore(now) && server_start.isAfter(now)){                
                ...
                Functions.printLog("TM[Delayer]: INF - Running command["+FixClient.BASH_CMD+"]");                        
                try {
                    Runtime.getRuntime().exec(FixClient.BASH_CMD + " stop");
                    Functions.printLog("TM[Delayer]: INF - FIX Status manager stopped");           
                    
                } catch (IOException ex) {
                    Functions.printLog("TM[Delayer]: ERR - Failed to stop FIX Status manager");           
                }
                ...
                //NB REVISIONED
                DBMakeQuery.setServerOnline(jdbc_connection, null, broker_status, trading_status);
                if(!FixClient.rate_mode.equalsIgnoreCase("auto")) DBMakeQuery.setServerOnline(jdbc_connection, FixClient.jdbc_demo_name, broker_status, trading_status);
                ...
                //NB REVISIONED
                if(!FixClient.rate_mode.equalsIgnoreCase("auto")) DBMakeQuery.forceCloseOrdersByStatus(jdbc_connection, FixClient.jdbc_demo_name, "open");
                ...
                //NB REVISIONED
                String tResult_real = DBMakeQuery.callGetForMarketClose(jdbc_connection, null);
                ...
                try {                            
                    ...
                    Functions.printLog(
                            "TM[Delayer] WRN - Failed to fall a sleep"
                            );
                }
                ...
                Functions.printLog("TM[Delayer] INF - FIX SNOOZES Zzz-zz-z");                      
                new Emailer().sendEmail(FixClient.app_id, "["+new Date().toString()+"] FIX SNOOZES Zzz-zz-z");
            
                if(FixClient.rate_mode.equalsIgnoreCase("auto")){
                    Functions.printLog("TM[Delayer]: INF - AUTO fix Starting sequence intiated ");
                    
                    FixClient.autoFix = new FixAutoRate(FixClient.fixClientProperties, DBMakeQuery.getClosingPrice(jdbc_connection, null), server_stop, server_start);
            ...
            //if server_start time is passed, but status is offline: go online, shedule next on\off time and start fix
                ...
                try {
                    Runtime.getRuntime().exec(FixClient.BASH_CMD + " restart");
                    Functions.printLog("TM[Delayer]: INF - FIX Status manager started");
                    
                } catch (IOException ex) {
                    Functions.printLog("TM[Delayer]: ERR - Failed to start FIX Status manager");           
                }
