...
public class HTTPURLConnection {
    public static /*final*/ long RAW_ROUND_PRECISION;// = (long)Math.pow(10, 5); //offset 5 point precision[FIXS max precision]
    
    private static /*final*/ String USER_AGENT;// = "hodiko.v@inbox.lv";
    private static /*final*/ String SERVICE_PROVIDER_URL;// = "http://www.random.org/integers/";
    ...
    //init
    public HTTPURLConnection(Properties randomORGProperties){
        ...
        this.RAW_ROUND_PRECISION = (long)Math.pow(10, Integer.parseInt(randomORGProperties.getProperty("fix_raw_precision")));
    ...
    public static boolean communicate2RandomOrg(){        
        ...
        if(backoff.isRetryOk() && quota < 0){                
            ...
            if(quota < 0){
                backoff.backoff();
                System.out.println("HTTPURLConnection:[comm2RandOrg] BACKOFFING ERROR Server not available or quota negative");
                return false;
            }
            ...
            if(!receivedSeedNums.isEmpty()){
                backoff.reset();
                System.out.println("HTTPURLConnection:[comm2RandOrg] INFO Received positive quota["+quota+"]. Received random numbers. Resetting backoff counter.");
                return true;

            }else{
                backoff.backoff();
                System.out.println("HTTPURLConnection:[comm2RandOrg] ERROR Failed to get number list from server. Received quota["+quota);
                return false;
            }
    ...
    //public static void generateRandomNumberList(){
        ...
        for(int i = 0; i < size; i++){
            randomList.add(
                    Functions.randLong(min, max, randomCore)
                    );
        }
    ...
    //public static void generateRandomNumberList(){
        ...
        for(int i = 0; i < size; i++){
            randomList.add(
                    Functions.randInt(min, max, randomCore)
                    );
        }
        //receivedNums = randomList;
        return randomList;
